...
def find_ingredient_relationships(df, min_support=0.003):  # Giảm min_support để tìm thêm tập phổ biến
...
def extract_ingredients_from_recipe(recipe_text):
    """Trích xuất nguyên liệu từ văn bản công thức"""
    # Tìm các nguyên liệu phổ biến trong văn bản
    common_ingredients = [
    ...
    # Tìm các nguyên liệu xuất hiện trong văn bản
    found_ingredients = []
...
def main():
    ...
    for ingredients in test_ingredients:
        ...
        for i, (_, recipe) in enumerate(recommendations.iterrows(), 1):
            ...
            ingredients_in_text = extract_ingredients_from_recipe(recipe['ingredients'])
            additional_ingredients = [ing for ing in ingredients if ing in ingredients_in_text and ing not in matching]
            if additional_ingredients:
                print(f"   Nguyên liệu có thể có trong văn bản: {', '.join(additional_ingredients)}")
